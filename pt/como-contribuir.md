...
**Português** · [English](../en/how-to-contribute.md)
...
O acervo tem uma árvore por idioma: `pt/` e `en/`, completas e independentes, mais `dados/` para os dados brutos que as duas citam. Nomes de arquivo são minúsculos, sem acento e com hífen, e são traduzidos junto com o texto.

Antes de criar um arquivo novo, leia [estrutura do acervo](estrutura.md). São dez minutos que evitam a bagunça que nenhum acervo bilíngue sobrevive.
...
Toda publicação identifica modelo e revisor, conforme a [ficha de responsabilidade](creditos-editoriais.md). Informe o modelo que realmente participou e não declare revisão, aprovação ou verificação independente que não ocorreu.
...
Toda contribuição é creditada com nome, inclusive a que não passa por código: tradução, revisão, uma fonte melhor, um erro apontado, uma ideia de recorte.

O crédito aparece em três lugares: no rodapé de cada documento, com modelo e revisor; na tabela de [colaboradores](colaboradores.md), com o que cada pessoa já fez; e nas notas de cada [edição](edicoes.md), com quem contribuiu naquele período. Você mesmo pode pedir seu registro, sem esperar que alguém se lembre — as instruções estão na página de colaboradores.

## Revisão e licença

Androquimera mantém a direção editorial inicial. Alterações podem ser discutidas e revisadas publicamente.

Ao propor a inclusão de texto de sua autoria, indique que ele pode ser distribuído sob a [CC BY 4.0](licenca.md) usada pelo projeto. Identifique material de terceiros e sua licença; só inclua conteúdo que você tenha direito de compartilhar.
